...
from main import Extrat
from filtrar_spam import ExecuteAPP
...
class Execute:
    ...
    def start():
        ...
        extract_rel_param = execution.parameters.get("extract_rel")
        if not extract_rel_param:
            extract_rel = False
        else:
            extract_rel = True if str(extract_rel_param).lower() in ['true', '1', 'yes', 'y'] else False

        filter_spam_param = execution.parameters.get("filter_spam")
        if not filter_spam_param:
            filter_spam = False
        else:
            filter_spam = True if str(filter_spam_param).lower() in ['true', '1', 'yes', 'y'] else False

        if (not filter_spam) and (not extract_rel):
            raise ValueError("Pelo menos um dos parâmetros 'extract_rel' ou 'filter_spam' deve ser verdadeiro.")
        
        if extract_rel:
            api = Extrat(
                username=maestro.get_credential(label=crd_param, key="user"),
